...
const BRANCH = 'For linking to a main website';
...
function deathSetup() {
	...
	//log score in data base ig higher than old high score
	/*logScore();
	function logScore() {
		firebase.database().ref('users/'+username+' '+uid).once('value', readHighScore, fb_error);
		if (snapshot.val() != null){
			console.log(snapshot.val());
			firebase.database().ref('users/'+username+' '+uid).update({
				UndUnlHighScore: 0,
			});
		} else {
			function readHighScore(snapshot) {
				highScore = snapshot.val();
				if (score > highScore){
					highScore = score;
					console.log('logging data in firebase');
					firebase.database().ref('users/'+username+' '+uid).update({
						UndUnlHighScore: highScore,
					});
				}
		}
		}
		// look for and print an error from fire base if one is given
		function fb_error(error){
			console.error("Fire base "+error)
		}
	}*/
}
//restets that game
